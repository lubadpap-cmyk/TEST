...
def init_db():
    ...
    # Create traps table (monitoring usernames)
    cursor.execute("""
    ...
    # Create dictionary table for dictionary word lookup
    cursor.execute("""
    ...
    # Migrations: Add language and referrals_count if they don't exist
    try:
    ...
    # Populate dictionary database
    populate_dictionary()

def populate_dictionary(force=False):
    """
    Populate (or refresh) the local dictionary table.
    If `force` is True, existing entries will be removed and replaced.
    By default this routine will try multiple upstream sources to build a richer
    wordlist useful for username generation and rating.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    # Try multiple sources to build a larger, but still useful, dictionary
    sources = [
    ...
    for url in sources:
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                # Keep words of length 3..10 (dictionary pieces useful for usernames)
                for w in data:
                    w = w.strip().lower()
    ...
    # Fallback list of very common words
    fallback_words = [
...
def add_user(user_id, username):
    ...
    try:
        cursor.execute(
            "INSERT OR IGNORE INTO users (user_id, username, is_premium, attempts_left, last_reset, language, referrals_count) VALUES (?, ?, 0, ?, ?, 'ru', 0)",
